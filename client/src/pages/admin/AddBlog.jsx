    ...
    const generateContent = async () => {
        ...
        try {
          ...
          if (response.data.success) {
            const generatedContent = response.data.content || response.data.data?.content || '';
            ...
            quillRef.current.root.innerHTML = marked(generatedContent);
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex-1 bg-blue-50/50 text-gray-600 h-full overflow-scroll'>
        <div className='bg-white w-full max-w-3xl p-4 md:p-10 sm:m-10 shadow rounded'>
            ...
            <input type="text" placeholder='Type here' required className='w-full max-w-lg mt-3 p-2 
            border border-gray-300 outline-none rounded' onChange={(e)=> setTitle(e.target.value)} 
            value={title}/>

            <p className='mt-4'>Sub Title</p>
            <input type="text" placeholder='Type here' required className='w-full max-w-lg mt-3 p-2 
            border border-gray-300 outline-none rounded' onChange={(e)=> setSubTitle(e.target.value)} 
            value={subTitle}/>
